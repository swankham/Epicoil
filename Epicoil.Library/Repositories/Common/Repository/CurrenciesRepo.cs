...
namespace Epicoil.Library.Repositories.Common
{
    public class CurrenciesRepo : ICurrenciesRepo
    {
        public IEnumerable<CurrencyModel> GetAll()
        {
            ...
            string sql = string.Format(@"SELECT SourceCurrCode as CurrencyCode, MAX(EffectiveDate) AS EffectiveDate, MAX(CurrentRate) as CurrentRate
		                                        FROM CurrExRate
		                                        WHERE TargetCurrCode = 'THB' and RateGrpCode Like '%BUY%'
		                                        GROUP BY SourceCurrCode
                                            UNION ALL
                                            SELECT curr.CurrencyCode, GETDATE() AS EffectiveDate, 
	                                            ISNULL((select top 1 CurrentRate
		                                        from CurrExRate
		                                        where SourceCurrCode = curr.CurrencyCode
		                                        order by EffectiveDate desc),1) as CurrentRate
		                                        FROM Currency curr
