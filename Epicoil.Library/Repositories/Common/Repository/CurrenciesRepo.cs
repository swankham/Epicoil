...
namespace Epicoil.Library.Repositories.Common
{
    ...
    {
        public IEnumerable<CurrenciesModel> GetAll()
        {
            IEnumerable<CurrenciesModel> result = new List<CurrenciesModel>();
            ...
            result = Repository.Instance.GetMany<CurrenciesModel>(sql);
            return result;
        }

        public CurrenciesModel GetByID(string code)
        {
            string sql = string.Format(@"SELECT SourceCurrCode as CurrencyCode, CurrentRate, MAX(EffectiveDate) AS EffectiveDate
		                                            FROM CurrExRate
		                                            WHERE TargetCurrCode = 'THB' AND RateGrpCode Like '%BUY%'
            ...
            return Repository.Instance.GetOne<CurrenciesModel>(sql);
        }
    }
}