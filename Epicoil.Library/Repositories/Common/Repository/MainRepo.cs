...
namespace Epicoil.Repositories
{
    public class MainRepo : IMainRepo
    {
        ...
        {
            string sql = string.Format(@"SELECT * FROM ucc_menus WHERE ParentMenuID = {0} AND MenuLevel = {1} ORDER BY Sequence ASC", ParentID, MenuLevel);
            return Repository.Instance.GetMany<MenuModel>(sql);
        }

        public IEnumerable<ItemMenuModel> GetAllItem(int ParentID)
        {
            string sql = string.Format(@"SELECT * FROM ucc_menus WHERE ParentMenuID = {0} AND Enabled = 1 AND MenuLevel = 4 ORDER BY Sequence ASC", ParentID);
