...
namespace Epicoil.Appl.Presentations.StoreInPlan
{
    public partial class ReceiveArticleDialog : BaseSession
    {
        ...
        private bool ValidateContent()
        {
                    ...
                    if (j == 6 || j == 7 || j == 8 || j == 9)
                    {
                        try
                        {
                            ...
                            if (rowIndex != i)
                            {
                                ...
                                tbllblMsg.Text = "Article No is duplicate.";
                                return true;
                            }
                            ...
                            {
                                dataGridView1.CurrentCell = dataGridView1[j, i];
                                tbllblMsg.Text = "Please fill data on current focus.";
                                return true;
                            }
                            ...
                            {
                                dataGridView1.CurrentCell = dataGridView1[j, i];
                                tbllblMsg.Text = "This Article number is duplicate.";
                                return true;
                            }
                            else
                            {
                            ...
                            var WhseByRow = _repoWhse.GetByID(dataGridView1.Rows[i].Cells["place"].Value.GetString());
                            if (WhseByRow == null)
                            {
                                dataGridView1.CurrentCell = dataGridView1[j, i];
                                tbllblMsg.Text = "This location dose not exist.";
                                return true;
                            }
                            else
                            {
            ...
            {
                error = true;
                tbllblMsg.Text = "Receipt weight must be equal Actual receipt weight!.";
            }
            else
            {
                tbllblMsg.Text = "";
        ...
        private void dataGridView1_CellValidating(object sender, System.Windows.Forms.DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (e.ColumnIndex == 8 || e.ColumnIndex == 7) // 1 should be your column index
            {
                ...
                {
                    e.Cancel = true;
                    tbllblMsg.Text = "Please enter numeric.";
                }
                else if (Convert.ToString(e.FormattedValue) == "0")
                {
                    e.Cancel = true;
                    tbllblMsg.Text = "Can't fill be zero.";
                }
                else
                {
                    tbllblMsg.Text = "";
