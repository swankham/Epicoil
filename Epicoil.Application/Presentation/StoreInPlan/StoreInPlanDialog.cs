...
{
    public partial class StoreInPlanDialog : Form
    {
        private readonly IStoreInPlanRepo _repo;
        public StoreInPlanHead HeadContent;
        public int status;
        private string Type;
        ...
        private void ListToGrid(IEnumerable<StoreInPlanDialogModel> item)
        {
            int i = 0;
            string Status = "";
            if (status != 2)
            ...
            foreach (var p in item)
            {
                ...
                dgvList.Rows.Add(p.StoreInPlanId, p.StoreInPlanNum, p.InvoiceNum, p.SupplierName, p.InvoiceDate, Status);
                if (i % 2 == 1)
                {
